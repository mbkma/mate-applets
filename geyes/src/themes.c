...
theme_dirs_create (void)
{
        ...
        theme_directories[1] = g_build_filename (g_get_user_config_dir (),
                                                 "mate", "geyes-themes", NULL);
                   ...
                   gpointer          data)
{
        ...
        if (!g_ascii_strncasecmp (theme, eyes_applet->theme_dir, strlen (theme))) {
                g_free (theme);
                return;
        }
        ...
        g_settings_set_string (eyes_applet->settings,
                               GEYES_SETTINGS_THEME_PATH_KEY, theme);
...
phelp_cb (GtkDialog *dialog)
{
        ...
        if (error) {
                GtkWidget *error_dialog
                        = gtk_message_dialog_new (NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
